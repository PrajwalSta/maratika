    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" action="{{route('saveonroadItinerary')}}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <label for="itinerary_title" class="col-form-label">Itinerary DayTitle:</label>
              <input type="text" class="form-control" id="itinerary_title" name="itinerary_title">
            </div>
            ...
            <div class="form-group">
              ...
              <input type="hidden" name="tour_id" value="{{$onroaditineraryId}}">
              <div class="form-group">
                <label for="itinerary_description" class="col-form-label">Brief  Description about the day:</label>
                <textarea type="text" class="form-control" id="itinerary_description" name="itinerary_description"></textarea>
              </div>
        </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" action="{{route('saveonroadItinerary')}}" enctype="multipart/form-data">
            ...
            <div class="form-group">
              <label for="es_itinerary_title" class="col-form-label">Itinerary DayTitle:</label>
              <input type="text" class="form-control" id="es_itinerary_title" name="es_itinerary_title">
            </div>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">OnRoad Itinerary 
...
{!! $itinerary->itinerary_title !!}
        @endswitch
          ...
          <p>
             @switch(app()->getLocale())
                      @case('es')
{!! strip_tags($itinerary->es_itinerary_description) !!}
                    @break
                      @case('de')
{!! strip_tags($itinerary->ger_itinerary_description) !!}
 
                      @break
                      @case('fr')
{!! strip_tags($itinerary->fr_itinerary_description) !!}
                      @break 
                      @default
{!! strip_tags($itinerary->itinerary_description) !!}
          ...
          <table>
            <tr>
              <td class="text-right">
                <a href="/edit-onroadItinerary/{{$itinerary->id}}/{{$onroaditineraryId}}" class="btn btn-success">Edit</a>
            </td>
                <td class="pull-right">
                    <form action="/delete-onroaditinerary/{{$itinerary->id}}" method="post" class="form-horizontal">
                        @csrf
                            @method('delete')
